...
import 'package:bt_habitat_ft_organizacion/moment/delete/bloc/delete_moment_bloc.dart';
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider<ComponentBloc>(
            create: (context) => ComponentBloc(workshopId, moment.id)
              ..add(ListComponentStarted())),
        BlocProvider<DeleteMomentBloc>(
            create: (_) => DeleteMomentBloc(workshopId)),
      ],
      child: _Moment(
...
class _Moment extends StatefulWidget {
  ...
  @override
  __MomentState createState() => __MomentState();
}

class __MomentState extends State<_Moment> with TickerProviderStateMixin {
      ...
      child: Row(
        children: <Widget>[
          ...
          IconButton(
              ...
              onPressed: () => BlocProvider.of<DeleteMomentBloc>(context)
                  .add(DeleteMomentStarted(widget.moment.id))),
          PopupMenuButton(
  ...
  Widget _buildBodyMoment() {
    return BlocBuilder<ComponentBloc, ComponentState>(
      builder: (context, state) {
        if (state is ListComponentSuccess) {
          return AnimatedSize(
